...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--heuristic-config", required=True, help="Path to heuristic config JSON")
...
def main() -> None:
    ...
    raw = load_all_data(args.input_data)
    validate_all_data(raw)
    featured = prepare_features(raw)

    heuristic = build_heuristic(args.heuristic_config)
    featured["NFL_production_value"] = heuristic.score(featured)

    train_df, test_df, split_info = _split_data(
        featured, seed=args.split_seed, time_split_year=args.time_split_year
    )

    summary_df = score_summary(featured)
    calibration_df = calibration_table(featured, bins=args.calibration_bins)
    ranking_df = rank_stability(train_df, test_df)
    top_df = top_players(featured)

    write_table(summary_df, output_dir / "score_summary.csv")
    write_table(calibration_df, output_dir / "calibration_table.csv")
    write_table(ranking_df, output_dir / "ranking_stability.csv")
    write_table(top_df, output_dir / "top_players.csv")
    write_table(featured, output_dir / "scored_players.csv")

    save_score_distribution(featured, output_dir / "plots" / "score_distribution.png")
    save_calibration_plot(calibration_df, output_dir / "plots" / "calibration_curve.png")
    ...
    manifest = {
        ...
        "data": {
            "input_path": str(Path(args.input_data).resolve()),
            "slice": data_slice,
        },
